...
- Standard techniques like stochastic volatility, rough volatility, or GARCH typically involve a minimum of four parameters when we include the drift (and these methods don't replicate q-variance anyway). That means you need to do something different (don't just rename one of the parameters as a fixed constant!).
